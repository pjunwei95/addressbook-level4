...
** 3) ** LOGIN ** with the Usernam - `admin` and Password - `admin` . +
...
** 9) Enter username as **SampleData**. `S` and `D` must in uppercase. `SampleData` is case-sensetive. Enter any password. +
...
*COMMAND 3* - `clear` +  press `ESC` / press `->` + `Space` /
press `Tab` + `Space` / click `X` / click `Cancel` /
